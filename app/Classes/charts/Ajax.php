...
class Ajax {
  ...
  const DATA_URL            = 'https://min-api.cryptocompare.com/data/histoday?fsym=%s&tsym=%s&limit=%d&aggregate=%d&allData=%s';
  ...
  public function ajaxSymbolAutocomplete() {
    ...
    if ($searchTerm) {
      foreach ($this->env->coins as $id => $coin) {
        // if (strpos(strtolower($coin->symbol), $searchTerm) !== FALSE) {
          if(strtolower($coin->symbol)==$searchTerm) {
          $result[] = [
            'id' => $id,
            'symbol' => $coin->symbol,
            'text' => $coin->name,
