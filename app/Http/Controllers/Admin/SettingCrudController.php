...
class SettingCrudController extends CrudController
{
    public function setup()
    {
        ...
        |--------------------------------------------------------------------------
        */
        $this->crud->setModel('App\Settings');
    ...
    public function edit($id)
    {
      ...
      //  $this->crud->addField((array) json_decode($this->data['entry']->field)); // <---- this is where it's different
        $value=(array) json_decode($this->data['entry']->field);

        if($settings->key=='currency')
        {
                $sitecurrency=array();
       
                $currencylist = Currency::where([['status','1'],['name','!=','PrimaryCoin']])->get();
                foreach ($currencylist as $currency)
                {
   
                    $sitecurrency[$currency['name']] = $currency['token'];
                }
           
            $value['options']=(object)$sitecurrency;    
        }
