...
class QtumdWalletNotifyController extends Controller {
	use TransactionProcess;

	public function transaction_qtum(Request $request) {
		$txn_hash= $request->txid;
	
	

		$blockInfo = Qtumd::gettransaction($txn_hash);
		...
		try {
			if (count($response->original) > 0) {
				...
				if (count($response->original['details']) > 0) {
					...
					$details1 = Userpayaccounts::where('qtum_label', $account1)->whereNotNull('qtum_label')->first();

					if (count($details1) > 0) {
						$balance_before = CryptoPaymentBase::crypto_getQTUMWalletBalanceByAccount($account1);
               ...
               $transaction=$this->createTransaction($details1->user_id,$details1->currency_id,$amount1,$type,"pending","cashpoint","NULL",'Notify',"NULL","NULL","NULL","NULL",0,0,$txn_hash,json_encode($response->original),$array);
					...
					if (count($response->original['details']) > 1) {
						...
						$details2 = Userpayaccounts::where('qtum_address', $address2)->first();
						if (count($details2) > 0) {
							$balance_before = 0;
							$balance_before = 0;
