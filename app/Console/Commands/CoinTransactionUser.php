...
class CoinTransactionUser extends Command
{
  use Common;
  /**
   ...
   * @var string
   */
  protected $signature = 'exchange:gettransactionsuser {userid}';
   ...
   * @var string
   */
  protected $description = 'Coin Transaction User';
...
//sowmi
  ...
  public function handle()
  {
    ...
    $coin = array('ETH', 'BTC', 'LTC', 'BCH', 'QTUM', 'RIPPLE');
    ...
    for ($i = 0; $i < count($coin); $i++) {
      ...
      $list = Userpayaccounts::where('paymentgateways_id', $pg->id)->where('user_id', $userId)->orderby('id', 'ASC')->get();
