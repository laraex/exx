<h4>{{ trans('forms.my_bank_wire_gbp') }}</h4>
@if(count($bankwire_gbp_result) > 0)
   @include('home._mypayaccounts_bankwire_gbp_details')
@endif
<p>
<a href="" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#bank_inr">{{ trans('forms.add_bank_account') }}</a>                       
</p>
<hr>
 <!-- Modal -->
  <div class="modal fade" id="bank_inr" role="dialog"> 
    <div class="modal-dialog">
    
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">{{ trans('forms.gbp_bank_details_form') }}</h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body">
             <form method="post" id="bankwire_inr_form" action="{{ url('myaccount/payaccounts')}}">
            {{ csrf_field() }}
